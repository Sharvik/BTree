...
public class ContenedorDeEnteros {
    ...
    private int numElem; // Numero de datos en el Multirrama
    private int Orden; // Orden del arbol B
    private int tamañoDatos; // Tamaño de cada dato almacenado
    ...
    private class Nodo {
        ...
        private void deByte(byte[] datos) {
            ...
            for (int i = 0; i < numElei; i++)
                clavei[i] = Conversor.toma(datos,
                            baseClaves + i * tamañoDatos, tamañoDatos);
            for (int i = 0; i <= numElei; i++) {
    ...
    /**
     * Clase auxiliar que permite devolver una pareja formada por una clave y el
     * enlace asociado
     */
    ...
    /**
     * Extrae del contenedor el elemento pasado por parametro, si no se
     * encuentra no se altera el contenedor.
     *
    ...
    public boolean extraer(int o) {
        ...
        Stack<InfoPila> pila = new Stack<InfoPila>();
        if (!buscar(alma, pila)) {
            return false; // Salimos sin hacer nada
        }
        fichero.adjunto(1, --numElem);
     ...
     * @return verdadero si el valor pasado por parametro pertenece al
     * contenedor y falso en caso contrario.
     */
    ...
    /**
     * Devuelve un vector de enteros ordenados de menor a mayor con los
     * elementos que se encuentren en el contenedor.
     *
