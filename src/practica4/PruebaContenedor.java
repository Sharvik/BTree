	...
	public static void main(String[] args) throws Exception {
		...
		for(int x = 0; x<Orden.length; x++){
			...
			for(int i=0;i<desordenado.length;i++){
				if(!b.insertar(desordenado[i])) System.out.println("El elemento '" + desordenado[i] + "' no se ha insertado");
			}
			...
			for(int i=0;i<array.length;i++) {
				...
				if(ordenado[i]!=array[i]) System.out.println("No se ha creado el array de elementos correctamente");
				// Buscamos todos los elementos del contenedor
				if(!b.buscar(desordenado[i])) System.out.println("No se ha encontrado el elemento '" + desordenado[i] + "' en el contenedor");
				if(!b.extraer(desordenado[i])) System.out.println("No se ha extraido el elemento '" + desordenado[i] + "' del contenedor");
			}
			...
			if(b.cardinal()!=0) System.out.println("El contenedor no se ha vaciado correctamente");
			...
			if(b.cardinal()!=1) System.out.println("No se ha insertado el elemento '" + array[0] + "' en un contenedor vacio");
			// Extraemos el '0' del contenedor
			b.extraer(0);
			// Creamos un array con los elementos del contenedor
			array = b.elementos();
			// Comprobamos que el contenedor este vacio
			if(b.cardinal()!=0) System.out.println("No se ha extraido el elemento '" + array[0] + "' en un contenedor con un elemento");
			...
			if(b.cardinal()!=1) System.out.println("El elemento anteriormente extraido no lo inserta en la ultima posicion del contenedor");
			// Buscar un elemento que no estÃ¡ en el contenedor
			if(a.buscar(11)) System.out.println("Se encontrÃ³ un elemento que no estÃ¡ en el contenedor");
			// Extraemos un elemento que no estÃ¡ en el contenedor
			if(a.extraer(19)) System.out.println("Se extrajo un elemento que no estÃ¡ en el contenedor");
			b.cerrar();
		}
		...
		try {
			...
			salida4.write("Primera prueba: \nTiempos de las inserciones en el contenedor: \n");
			for (int t = 0; t < arbolesb.length; t++ ) {
				salida4.write("\n Orden " + Orden[t] + ":\n");
				princ = System.nanoTime();	//	Inicializamos con el tiempo del sistema
				for(int i = 0;i < datos.length;i++){	//Tenemos en cuenta 100000 elementos
					arbolesb[t].insertar(datos[i]);
					// Usamos el modulo para dividir los rangos a probar (de 10000 en 10000)
			...
			System.out.println("Inserciones realizadas --> Proceso total: 25%");

			//Comenzamos el algoritmo para las extracciones y el calculo de sus tiempos
			
			salida4.write("\n");
			salida4.write("Segunda prueba: \nTiempos de las extracciones del contenedor: \n");
			for (int t = 0; t < arbolesb.length; t++ ) {
				salida4.write("\n Orden " + Orden[t] + ":\n");
				princ = System.nanoTime();	//	Inicializamos con el tiempo del sistema
			...
			System.out.println("Extracciones realizadas --> Proceso total: 50%");
